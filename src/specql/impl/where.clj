...
(defn- where-map [table-info-registry base-table path->table record path-prefix]
    ...
    (do
      ...
      (as-> (reduce
             (fn [where [column-keyword value]]
               (let [{column-table :table} (path->table (into path-prefix [column-keyword]))]
                 (println "COL-KW: " column-keyword ", COL-TABLE: " column-table)
                 ;; If column is a joined table, it has a mapping in path->table.
                 ;; Recursively create clauses for the value
                 (if (not= base-table column-table)
                   (let [[sql params] (sql-where table-info-registry column-table path->table value
...
(defn sql-where
  ([table-info-registry base-table path->table record]
   (sql-where table-info-registry base-table path->table record []))
  ([table-info-registry base-table path->table record path-prefix]
   (if (op/combined-op? record)
     ...
     (where-map table-info-registry base-table path->table record path-prefix))))
